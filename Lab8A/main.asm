...
CONTINUELOOP:								 ;A loop to keep asking the user if they want to continue until stated no
	endl
	...
	.WHILE al == 'y' || al == 'Y' 				         ;while the user's answer is yes to continue to play the game, do the following
		.IF balance == 0					 ;check to see if the balance is 0, if it is, 
			mov EDX, OFFSET zeroBalanceMsg		
			call writeLine
			jmp ENDGAME					 ;then end game. 
		.ENDIF
